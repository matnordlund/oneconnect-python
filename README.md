...
- **Profile manager:** A profile manager window opens at startup so you can add, edit, delete, and connect from the window (select a profile and click Connect or double-click). You can also open it from the tray via “Manage profiles”. On Ubuntu/GNOME the tray icon only appears if the “AppIndicator and KStatusNotifierItem Support” extension is enabled; without it, use the profile manager window to connect.
